...
OPERADORES = {
    '+': 'tk_suma', '-': 'tk_resta', '*': 'tk_mult', '/': 'tk_div',
    '**': 'tk_potencia', '%': 'tk_modulo', '//': 'tk_div_entero',
    '=': 'tk_asig', '==': 'tk_eq', '!': 'tk_negacion', '!=': 'tk_distinto',
    '<': 'tk_menor', '<=': 'tk_menor_igual', '>': 'tk_mayor', '>=': 'tk_mayor_igual',
    'and': 'tk_and', 'or': 'tk_or', 'not': 'tk_not', 'is': 'tk_is', 'in': 'tk_in',
    '~': 'tk_negacion_bitwise', '<<': 'tk_shift_izq', '>>': 'tk_shift_der',
    '&': 'tk_and_bitwise', '^': 'tk_xor_bitwise', '|': 'tk_or_bitwise',
    '.': 'tk_punto', ':=': 'tk_asig_rapida'
}
...
PALABRAS_RESERVADAS = {
    ...
    'in': 'in', 'is': 'is', 'lambda': 'lambda', 'nonlocal': 'nonlocal', 'not': 'not',
    'or': 'or', 'pass': 'pass', 'mft': 'mft', 'raise': 'raise', 'return': 'return',
    'try': 'try', 'while': 'while', 'with': 'with', 'yield': 'yield',
    'print': 'print', 'range': 'range', 'match': 'match', 'case': 'case', 'type': 'type', '_': '_',

}

SIMBOLOS_ESPECIALES = {
    '(': 'tk_par_izq', ')': 'tk_par_der', ':': 'tk_dos_puntos', ',': 'tk_coma',
    '.': 'tk_punto', '"': 'tk_comilla_doble', "'": 'tk_comilla_simple', ';': 'tk_punto_coma',
    '{': 'tk_llave_izq', '}': 'tk_llave_der', '[': 'tk_corchete_izq', ']': 'tk_corchete_der',
    '!': 'tk_exclamacion', '@': 'tk_arroba', '=': 'tk_igual', '->': 'tk_flecha',
    '+=': 'tk_suma_asignacion', '-=': 'tk_resta_asignacion', '*=': 'tk_mult_asignacion',
    '/=': 'tk_div_asignacion', '//=': 'tk_div_entera_asignacion', '%=': 'tk_mod_asignacion',
    '@=': 'tk_arroba_asignacion', '&=': 'tk_and_asignacion', '|=': 'tk_or_asignacion',
    '^=': 'tk_xor_asignacion', '>>=': 'tk_shift_der_asignacion', '<<=': 'tk_shift_izq_asignacion',
    '**=': 'tk_potencia_asignacion', '#': 'tk_comentario', '\\': 'tk_barra_invertida'
}

# ... (el resto de las funciones en el archivo `afd.py`)
...
def afd_analizador_lexico(texto):
    ...
    while i < len(texto):
        ...
        if estado == 0:  # Estado inicial
            if caracter == '\n':  # Nueva línea	
                ...
                if i < len(texto):
                    espacios_inicio = 0
                    while i < len(texto) and texto[i] in ' \t' and texto[i] != '\n':
                        espacios_inicio += 1
                        i += 1
                    if i < len(texto) and texto[i] != '\n':
                        if espacios_inicio > nivel_indentacion_actual:
                            tokens.append(('INDENT', numero_linea, espacios_inicio))
                            pila_indentacion.append(espacios_inicio)
                            nivel_indentacion_actual = espacios_inicio
                        elif espacios_inicio < nivel_indentacion_actual:
                            while pila_indentacion and pila_indentacion[-1] > espacios_inicio:
                                tokens.append(('DEDENT', numero_linea, nivel_indentacion_actual))
                                pila_indentacion.pop()
                            nivel_indentacion_actual = pila_indentacion[-1] if pila_indentacion else 0
                    i -= 1
                        # No incrementamos `i` aquí para procesar el siguiente carácter correctamente
            elif es_letra(caracter):
                estado = 1
                lexema += caracter
            ...
            if es_letra(caracter) or es_digito(caracter):
                ...
                # Verificar si es palabra reservada antes que un identificador
    ...
    # Procesar tokens pendientes y niveles de indentación al final
    if lexema:
        if estado == 1:
            if lexema in PALABRAS_RESERVADAS:
                tokens.append((PALABRAS_RESERVADAS[lexema], numero_linea, columna - len(lexema)))
    ...
    while len(pila_indentacion) > 1:
        tokens.append(('DEDENT', numero_linea, 0))
        pila_indentacion.pop()
